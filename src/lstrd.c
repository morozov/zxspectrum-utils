...
/*	char *diskformat; //not used yet */
	...
	{	
		/* soubor otevřen a načten v paměti, teď by nemělo být moľné, aby nastala chyba */
		...
		switch (listmode)
		{
			case 2:
				/* 
				   reľim EXTRA SHORT (haven't equivalent in real TRDOS, but may be usefull for
				...
				for (trdosfile=0; trdosfile<directory[INFO_SEC*SEC_SIZE + OFFSET_FILES]; trdosfile++)
				{
					...
					switch (directory[trdosfile*16+8])
					{
						...
						case 'B':
							...
							len2 = /* délka basicu bez proměnných */ directory[trdosfile*16+9]+256*directory[trdosfile*16+10];
							break;
						default:
							len1 = /* adresa kam se soubor nahraje do RAM, není-li explicitně určeno jinak */
					...
					printf ("%s <%c>  %-13s%5i  %6i%7i   %3i    %3i\n"
							...
							, len2				/* délka basicu bez proměnných / délka v bytech */
...
/*		printf ("++ %i ++ %s \n",counter, argv[counter]); */
		...
		{
			/*
			   nepochopitelný parametr je tolerován jen jako poslední, to totiľ musí být
			   jméno souboru, jestli je platné, to se ukáľe později, pokud by byl nepochopitelný 
			   parametr nalezen dříve neľ na posledním místě, bude zobrazena nápověda
			*/
